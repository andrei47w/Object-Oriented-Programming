...
Tutorial::Tutorial(const std::string &title, const std::string &presenter, const Duration &duration, const int &noLikes,
                   const std::string &source, const int &noViews) {
    this->title = title;
...
bool Tutorial::operator==(const Tutorial &t) const {
...
bool Tutorial::operator<(int value) const {
    return this->noViews < value;
}


std::ostream &operator<<(std::ostream &out, const Tutorial &t) {
    out << t.getPresenter().c_str() << "," << t.getTitle().c_str() << "," << t.getNoLikes() << "," << t.getNoViews()
        << "," << t.getDuration().getMinutes() << "," << t.getDuration().getSeconds() << "," << t.getSource().c_str();
    return out;
}
...
std::istream &operator>>(std::istream &in, Tutorial &t) {
    ...
    if (tokens.size() != 7) // make sure all the song data was valid
        throw ValidationException("The number of fields is not ok!");
    t = TutorialFactory::MakeTutorial(trim(tokens[1]), trim(tokens[0]), trim(tokens[4]), trim(tokens[5]),
                                      trim(tokens[3]), trim(tokens[6]), trim(tokens[2]));

    return in;
}

Tutorial TutorialFactory::MakeTutorial(const std::string &title, const std::string &presenter, const std::string &m,
                                       const std::string &s, const std::string &noLikes, const std::string &source,
                                       const std::string &noViews) {
    std::string errors = "";
    std::string titleS = "";
